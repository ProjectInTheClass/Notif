...
class HistoryViewController: UIViewController, UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        return Array(Set( self.cardsViewController.cards.map{$0.historyFormattedDate})).count
    }
    
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return Array(Set(self.cardsViewController.cards.map{$0.historyFormattedDate})).sorted().reversed()[section]
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        let date = Array(Set(self.cardsViewController.cards.map{$0.historyFormattedDate})).sorted().reversed()[section]
        return self.cardsViewController.cards.filter{$0.historyFormattedDate == date}.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let date = Array(Set(self.cardsViewController.cards.map{$0.historyFormattedDate})).sorted().reversed()[indexPath.section]
        //cardsViewController.cards를 돌면서 각 날짜에 맞는 card를 필터를 걸어서 찾아내기 위한 변수
        let nowCell = self.cardsViewController.cards.filter{$0.historyFormattedDate==date}[indexPath.row]
        ...
        if(nowCell.source != nil){
            ...
            cardCell.dateLabel.text = nowCell.historyFormattedDate
